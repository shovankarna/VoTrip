 ...
 * returning the resolved UID is enough to confirm the whole chain works. There is no service layer
 * behind it because there is no work to do beyond reading the already-authenticated principal.
 */
@RestController
...
public class PingController {
    ...
    public PingResponse ping(@AuthenticationPrincipal FirebaseUserPrincipal principal) {
        return new PingResponse(principal.uid());
    }
}
